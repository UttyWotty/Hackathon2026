"""
Duration Deviation Analysis API.

This module provides the main API wrapper for duration deviation analysis,
...
def run_analysis_api(
    ...
    """Run duration deviation analysis and generate reports.

    This is the main entry point for the duration deviation analysis module,
    designed for programmatic access and LLM integration.

    Args:
        start_date: Start date in 'YYYY-MM-DD' format (optional)
        end_date: End date in 'YYYY-MM-DD' format (optional)
        machine_ids: List of equipment codes to analyze (optional)
        vendor_names: List of supplier names to analyze (optional)
        output_dir: Output directory for reports (optional, defaults to 'deviation_results')
    ...
    """
    try:
        logger.info("🚀 Starting Duration Deviation Analysis")

        # Set default output directory (centralized)
        if output_dir is None:
            from analysis.shared import get_output_dir

            output_dir = str(get_output_dir("deviation"))
        else:
        ...
        # Fetch data
        logger.info("📥 Fetching duration deviation data...")
        df = fetch_deviation_data(
            session=session,
        ...
        logger.info("✅ Duration Deviation Analysis completed successfully")
        ...
        logger.error(f"❌ Error in duration deviation analysis: {e}", exc_info=True)
        return {
            "status": "error",
            "error": str(e),
            "metrics": [],
...
def _save_metrics_to_csv(
        ...
        results_data.append(
            {
                ...
                "Average_CT": m.avg_duration,
                "Target_Duration": m.target_duration,
                "CT_Deviation": m.deviation,
    ...
    csv_path = os.path.join(output_dir, f"deviation_analysis_{timestamp}.csv")
...
def _save_html_report(
    ...
    html_content = generate_html_report(
        ...
        charts=charts,
        title="Duration Deviation Analysis Report",
    )

    html_path = os.path.join(output_dir, f"deviation_report_{timestamp}.html")
