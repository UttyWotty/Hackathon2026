...
This module groups equipment by TARGET_DURATION (same approved duration = same or similar
parts) and compares their duration efficiency within each group. This provides
...
# Tolerance for grouping approved durations (seconds) - exact match by default
TARGET_DURATION_TOLERANCE = 0.0


@dataclass
class ToolInGroup:
    """One equipment's performance within an approved duration group.
    ...
    Attributes:
        ...
        mean_efficiency_pct: Mean efficiency (TARGET_DURATION / DURATION - 1) * 100
...
class ApprovedCTGroup:
    """A group of equipment sharing the same TARGET_DURATION (same part).

    Attributes:
        target_duration: The shared approved duration (seconds)
        product_ids: Unique PRODUCT_IDs seen in this group
        product_names: Unique PRODUCT_NAMEs seen in this group
...
def build_target_duration_groups(
    df: pd.DataFrame,
    tolerance: float = TARGET_DURATION_TOLERANCE,
) -> Dict[float, pd.DataFrame]:
    """Group data by TARGET_DURATION value.

    Args:
        df: DataFrame with TARGET_DURATION, tool_id, efficiency_pct columns
        tolerance: Tolerance for duration matching (0 = exact match)

    Returns:
        Dict mapping target_duration value to DataFrame of shots in that group
    """
    if tolerance == 0.0:
        # Exact match: round to 2 decimal places to handle float precision
        df = df.copy()
        df["ct_group"] = df["TARGET_DURATION"].round(2)
    else:
        # Bucket by tolerance
        df = df.copy()
        df["ct_group"] = (df["TARGET_DURATION"] / tolerance).round(0) * tolerance
    ...
    logger.info(
        "Found %d approved duration groups with >= %d equipment",
        len(groups),
...
def analyze_group(
    ...
    """Analyze equipment performance within one approved duration group.
...
def compare_tools_by_target_duration(
    df: pd.DataFrame,
    tolerance: float = TARGET_DURATION_TOLERANCE,
) -> List[ApprovedCTGroup]:
    """Run tool comparison across all approved duration groups.
    ...
    """
    logger.info("Comparing tool performance within approved duration groups...")
