...
duration efficiency and tool consistency.
...
def benchmark_suppliers(tool_metrics: pd.DataFrame) -> List[SupplierBenchmark]:
    ...
    # Group by supplier
    supplier_groups = tool_metrics.groupby("VENDOR_NAME")

    benchmarks = []

    for vendor_name, group in supplier_groups:
...
def get_supplier_comparison(
    benchmarks: List[SupplierBenchmark], vendor_names: List[str] = None
) -> List[Dict]:
    """Get comparison data for specific suppliers.

    Args:
        benchmarks: List of SupplierBenchmark objects
        vendor_names: Optional list of supplier names to compare

    Returns:
        List[Dict]: Comparison data for requested suppliers
    """
    if vendor_names:
        filtered = [b for b in benchmarks if b.vendor_name in vendor_names]
    else:
