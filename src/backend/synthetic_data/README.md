...
`SHOT_DATA` contract, so the existing sense tools run against it unmodified. Every defect
...
|---|---|---|
| `SHOT_DATA` | ~230,000 | The central fact table every sense tool reads |
...
| Equipment | Archetype | duration deviation | Stability | MTTR | MTBF | Detected by |
|---|---|---|---|---|---|---|
| MX-7103 | ct_drift | **12.7%** (1.9% to 24.0%) | 90.0% | 2.4 | 22.3 | deviation |
...
`MX-7103` is the demo headline. Its duration drifts from 1.9% to 24.0% above approved duration across
...
misses it entirely. Surfacing it requires reasoning across duration deviation and stability together,
...
**Durations are quantized to a 0.1-second grid, and normal cycles are drawn from a distribution
...
- the modal duration is unique per run
...
- Supplier-to-tooling-family mapping is simplified: `TYPE` mirrors `TYPE`, matching
