"""
Tests for the pure SHOT_DATA filter predicates.
...
def shots() -> pd.DataFrame:
    ...
    return pd.DataFrame(
        {
            "DURATION": [10.0, 12.0, 11.0, 0.0, None, 1000.0, 9.0, 13.0],
            "TARGET_DURATION": [10.0, 10.0, 10.0, 10.0, 10.0, 10.0, 0.0, 10.0],
...
class TestValidityFilter:
    def test_drops_null_and_non_positive_and_sentinel_rows(self, shots):
        ...
        # Rows dropped: CT=0, DURATION=None, DURATION=1000 (>= 999.9), TARGET_DURATION=0.
        assert len(kept) == 4
        assert kept["DURATION"].tolist() == [10.0, 12.0, 11.0, 13.0]

    def test_boundary_ct_is_excluded(self):
        frame = pd.DataFrame(
            {"DURATION": [MAX_VALID_DURATION], "TARGET_DURATION": [10.0]}
        )
        assert apply_validity_filter(frame).empty

    def test_just_below_boundary_is_kept(self):
        frame = pd.DataFrame(
            {"DURATION": [MAX_VALID_DURATION - 0.1], "TARGET_DURATION": [10.0]}
        )
...
class TestMembershipFilter:
    def test_equipment_subset(self, shots):
        kept = apply_membership_filter(shots, machine_ids=["MX-7103"])
        assert set(kept["MACHINE_ID"]) == {"MX-7103"}

    def test_wildcard_means_all_equipment(self, shots):
        # The sense tools pass ["*"] for "every machine".
        assert len(apply_membership_filter(shots, machine_ids=["*"])) == len(shots)

    def test_supplier_subset(self, shots):
        kept = apply_membership_filter(shots, vendor_names=["B"])
        assert set(kept["VENDOR_NAME"]) == {"B"}
...
class TestFilterShots:
    def test_result_is_ordered_by_shot_time(self, shots):
        ...
        assert out["SHOT_TIME"].is_monotonic_increasing
    ...
    def test_predicates_compose(self, shots):
        out = filter_shots(shots, start_date="2026-06-09", machine_ids=["MX-7103"])
        assert len(out) == 2
        assert set(out["MACHINE_ID"]) == {"MX-7103"}
...
class TestEndBoundDivergence:
    ...
    def late(self) -> pd.DataFrame:
        ...
        return pd.DataFrame(
            {
                "DURATION": [10.0, 10.0],
                "TARGET_DURATION": [10.0, 10.0],
                "SHOT_TIME": pd.to_datetime(
                    ["2026-06-10 12:00:00.000", "2026-06-10 23:59:59.500"],
                    format="ISO8601",
                ),
                "MACHINE_ID": ["MX-7101", "MX-7101"],
                "VENDOR_NAME": ["A", "A"],
            }
        )

    def test_deviation_bound_excludes_sub_second_tail(self, late):
        ...
        assert len(kept) == 1
        assert kept["SHOT_TIME"].iloc[0].hour == 12

    def test_efficiency_bound_is_midnight(self, late):
