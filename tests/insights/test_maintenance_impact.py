...
Verifies direction-aware impact labels (lower CT is better, higher output is better),
...
def test_lower_ct_after_maintenance_is_improvement():
    detail = classify_metric_impact("avg_ct", before=12.0, after=10.0)
    assert detail["impact"] == IMPACT_IMPROVED
    assert detail["change_pct"] == -16.67


def test_higher_ct_after_maintenance_is_degradation():
    detail = classify_metric_impact("avg_ct", before=10.0, after=12.0)
...
def test_small_change_is_neutral():
    detail = classify_metric_impact("avg_ct", before=100.0, after=101.0)
    assert detail["impact"] == IMPACT_NEUTRAL


def test_missing_values_are_unknown():
    assert classify_metric_impact("avg_ct", None, 10.0)["impact"] == IMPACT_UNKNOWN
    assert classify_metric_impact("avg_ct", 10.0, None)["impact"] == IMPACT_UNKNOWN
    assert classify_metric_impact("avg_ct", 0.0, 10.0)["impact"] == IMPACT_UNKNOWN


def test_overall_verdict_majority():
    result = compare_before_after(
        {"avg_ct": 12.0, "shots_per_day": 100.0, "downtime_minutes": 60.0},
        {"avg_ct": 10.0, "shots_per_day": 95.0, "downtime_minutes": 30.0},
    )
    assert result["metrics"]["avg_ct"]["impact"] == IMPACT_IMPROVED
...
def test_overall_unknown_when_no_data():
    result = compare_before_after({"avg_ct": None}, {"avg_ct": None})
