"""
Unit tests for the pareto_ct_deviations module.
Covers calculate_std_deviations, calculate_iqr_deviations, combine_deviation_methods,
and calculate_statistical_ct_deviations with happy-path, boundary, and error cases.
...
from analysis.rca.core_analysis.pareto_ct_deviations import (
    ...
    calculate_statistical_ct_deviations,
...
class TestCalculateStdDeviations:
    ...
    def test_happy_path_adds_columns(self) -> None:
        ...
        assert "CT_DEVIATION_FROM_MEAN_PCT" in result.columns
    ...
    def test_outlier_detected_for_extreme_value(self) -> None:
        ...
        result = calculate_std_deviations(df)
        outlier_row = result[result["CT"] == 50.0]
    ...
    def test_group_below_min_size_gets_nan(self) -> None:
        ...
        result = calculate_std_deviations(df)
        assert result["CT_DEVIATION_FROM_MEAN_PCT"].isna().all()
    ...
    def test_deviation_pct_sign(self) -> None:
        ...
        df = _make_shot_df(ct_values)
        result = calculate_std_deviations(df)
        above_mean_row = result[result["CT"] == 15.0]
        assert above_mean_row["CT_DEVIATION_FROM_MEAN_PCT"].iloc[0] > 0
...
class TestCalculateIqrDeviations:
    ...
    def test_happy_path_adds_columns(self) -> None:
        ...
        assert "CT_DEVIATION_FROM_MEDIAN_PCT" in result.columns
    ...
    def test_outlier_detected_for_extreme_value(self) -> None:
        ...
        df = _make_shot_df(ct_values)
        result = calculate_iqr_deviations(df)
        outlier_row = result[result["CT"] == 100.0]
    ...
    def test_group_below_min_size_gets_nan(self) -> None:
        ...
        df = _make_shot_df([10.0] * (MIN_GROUP_SIZE - 1))
        result = calculate_iqr_deviations(df)
        assert result["CT_DEVIATION_FROM_MEDIAN_PCT"].isna().all()
    ...
    def test_median_deviation_sign(self) -> None:
        ...
        df = _make_shot_df(ct_values)
        result = calculate_iqr_deviations(df)
        median_val = df["CT"].median()
        below_median = result[result["CT"] < median_val]
        if len(below_median) > 0:
            assert (below_median["CT_DEVIATION_FROM_MEDIAN_PCT"].dropna() < 0).all()
...
class TestCombineDeviationMethods:
    ...
    def test_happy_path_adds_composite_columns(self) -> None:
        """Combine adds OUTLIER_SCORE, CT_DEVIATION_PCT, CT_ISSUE_FLAG, CT_ISSUE_TYPE."""
        df = pd.DataFrame(
            {
                "CT": [10.0, 20.0, 30.0],
                ...
                "CT_DEVIATION_FROM_MEAN_PCT": [1.0, 10.0, 60.0],
                "CT_DEVIATION_FROM_MEDIAN_PCT": [1.0, 10.0, 55.0],
                "CT_DEVIATION_PERCENTILE": [1.0, 10.0, 65.0],
            }
        )
        result = combine_deviation_methods(df)
        assert "OUTLIER_SCORE" in result.columns
        assert "CT_DEVIATION_PCT" in result.columns
        ...
        df = pd.DataFrame(
            {
                ...
                "CT_DEVIATION_FROM_MEAN_PCT": [5.0],
                "CT_DEVIATION_FROM_MEDIAN_PCT": [5.0],
                "CT_DEVIATION_PERCENTILE": [5.0],
            }
        )
        result = combine_deviation_methods(df)
        ...
        df = pd.DataFrame(
            {
                ...
                "PERCENTILE_OUTLIER": [False],
                "ROLLING_OUTLIER": [False],
                "CT_DEVIATION_FROM_MEAN_PCT": [HIGH_DEVIATION_PCT + 1],
                "CT_DEVIATION_FROM_MEDIAN_PCT": [0.0],
                "CT_DEVIATION_PERCENTILE": [0.0],
            }
        )
        result = combine_deviation_methods(df)
        assert bool(result["CT_ISSUE_FLAG"].iloc[0]) is True
        ...
        df = pd.DataFrame(
            {
                ...
                "PERCENTILE_OUTLIER": [False],
                "ROLLING_OUTLIER": [False],
                "CT_DEVIATION_FROM_MEAN_PCT": [1.0],
                "CT_DEVIATION_FROM_MEDIAN_PCT": [1.0],
                "CT_DEVIATION_PERCENTILE": [1.0],
            }
        )
        result = combine_deviation_methods(df)
        ...
        df = pd.DataFrame(
            {
                ...
                "CT_DEVIATION_FROM_MEAN_PCT": [CRITICAL_DEVIATION_PCT + 10],
                "CT_DEVIATION_FROM_MEDIAN_PCT": [CRITICAL_DEVIATION_PCT + 10],
                "CT_DEVIATION_PERCENTILE": [CRITICAL_DEVIATION_PCT + 10],
            }
        )
        result = combine_deviation_methods(df)
        ...
        df = pd.DataFrame(
            {
                ...
                "PERCENTILE_OUTLIER": [False],
                "ROLLING_OUTLIER": [False],
                "CT_DEVIATION_FROM_MEAN_PCT": [deviation],
                "CT_DEVIATION_FROM_MEDIAN_PCT": [deviation],
                "CT_DEVIATION_PERCENTILE": [deviation],
            }
        )
        result = combine_deviation_methods(df)
        ...
        df = pd.DataFrame(
            {
                ...
                "PERCENTILE_OUTLIER": [False],
                "ROLLING_OUTLIER": [False],
                "CT_DEVIATION_FROM_MEAN_PCT": [2.0],
                "CT_DEVIATION_FROM_MEDIAN_PCT": [2.0],
                "CT_DEVIATION_PERCENTILE": [2.0],
            }
        )
        result = combine_deviation_methods(df)
    ...
    def test_missing_deviation_cols_defaults_zero(self) -> None:
        ...
        result = combine_deviation_methods(df)
        assert result["CT_DEVIATION_PCT"].iloc[0] == 0.0

    def test_approved_ct_blending(self) -> None:
        """APPROVED_CT column blends into CT_DEVIATION_PCT."""
        df = pd.DataFrame(
            {
                "CT": [20.0],
                "APPROVED_CT": [10.0],
                ...
                "PERCENTILE_OUTLIER": [False],
                "ROLLING_OUTLIER": [False],
                "CT_DEVIATION_FROM_MEAN_PCT": [30.0],
                "CT_DEVIATION_FROM_MEDIAN_PCT": [30.0],
                "CT_DEVIATION_PERCENTILE": [30.0],
            }
        )
        result = combine_deviation_methods(df)
        deviation = result["CT_DEVIATION_PCT"].iloc[0]
...
# ===================================================================
# calculate_statistical_ct_deviations (orchestrator)
# ===================================================================


class TestCalculateStatisticalCtDeviations:
    """Tests for the orchestrator calculate_statistical_ct_deviations."""
    ...
    def test_happy_path_all_columns_present(self) -> None:
        ...
        result = calculate_statistical_ct_deviations(df)
        expected_cols = [
            "STD_OUTLIER",
            "IQR_OUTLIER",
            "ZSCORE_OUTLIER",
    ...
    def test_custom_thresholds_propagate(self) -> None:
        ...
        result_strict = calculate_statistical_ct_deviations(
            df.copy(),
            std_threshold=1.0,
            iqr_multiplier=0.5,
            z_score_threshold=0.5,
        )
        result_loose = calculate_statistical_ct_deviations(
            df.copy(),
            std_threshold=5.0,
            iqr_multiplier=5.0,
    ...
    def test_multi_equipment_groups(self) -> None:
        ...
        assert set(result["EQUIPMENT_CODE"].unique()) == {"EQ_A", "EQ_B"}
