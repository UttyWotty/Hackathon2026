"""What-if savings simulation for cycle time improvements.

Computes the production time saved and the extra parts attainable if equipment ran at a
target cycle time (approved CT or best performer in its approved CT group) instead of
...
def compute_record_savings(
    ...
    Args:
        ...
        avg_ct: Observed average cycle time in seconds.
        target_ct: Cycle time to simulate (approved or group-best), in seconds.
    ...
    """
    if not shots or avg_ct is None or target_ct is None or target_ct <= 0:
        return {"applicable": False, "hours_saved": 0.0, "extra_parts_possible": 0}

    ct_gap = avg_ct - target_ct
    if ct_gap <= 0:
        return {"applicable": False, "hours_saved": 0.0, "extra_parts_possible": 0}

    seconds_saved = shots * ct_gap
    extra_parts = int(seconds_saved / target_ct) * max(cavities, DEFAULT_CAVITIES)
    return {
        "applicable": True,
...
def simulate_savings_records(records: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    Args:
        records: Dicts with shots, avg_ct, target_ct, and optional cavities keys;
            identifying keys (equipment_code, part_id, ...) are passed through.
