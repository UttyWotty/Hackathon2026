"""
CT Efficiency Result Serializers.
    ...
    for g in groups:
        for t in g.tools:
            rows.append(
                {
                    "Approved_CT": g.approved_ct,
                    "Part_IDs": "; ".join(g.part_ids[:5]),
                    "Part_Names": "; ".join(g.part_names[:5]),
...
def save_staleness_report_to_csv(
    ...
    for r in results:
        rows.append(
            {
                "Approved_CT": r.approved_ct,
                "Part_Names": "; ".join(r.part_names[:3]),
                ...
                "Current_Approved_CT": r.approved_ct,
                "Latest_Actual_Mean_CT": (
                    r.monthly_snapshots[-1].mean_ct if r.monthly_snapshots else ""
                ),
                "Suggested_CT": r.suggested_ct,
                "Reasoning": r.reasoning,
            }
        )

    df = pd.DataFrame(rows)
    csv_path = os.path.join(output_dir, f"approved_ct_staleness_{timestamp}.csv")
    df.to_csv(csv_path, index=False)
...
def save_supplier_shifts_to_csv(
    ...
    """
    rows = []
    for supplier_name, shifts in supplier_shifts.items():
        rows.append(
            {
