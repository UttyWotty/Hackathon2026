...
class DetectedShifts:
    ...
    Attributes:
        supplier_name: Which supplier this applies to
...
def _build_hourly_profile(df: pd.DataFrame) -> pd.Series:
    ...
    """
    df = df.copy()
    df["hour"] = df["LOCAL_SHOT_TIME"].dt.hour
...
def detect_shift_boundaries(
    ...
    Args:
        df: Full DataFrame with LOCAL_SHOT_TIME
...
def detect_shifts_per_supplier(
    ...
    Args:
        df: Full DataFrame with LOCAL_SHOT_TIME and SUPPLIER_NAME
    ...
    for supplier_name, supplier_df in df.groupby("SUPPLIER_NAME"):
        result = _detect_for_supplier(str(supplier_name), supplier_df)
        results[str(supplier_name)] = result
