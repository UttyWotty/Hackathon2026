"""
CT Efficiency Calculator.

This module contains functions for calculating cycle time efficiency metrics
...
def calculate_cycle_time_efficiency(df: pd.DataFrame) -> pd.DataFrame:
    """Calculate cycle time efficiency using APPROVED_CT as baseline.
    ...
    Formula:
        efficiency = APPROVED_CT / actual_CT
        efficiency_pct = (efficiency - 1) * 100
    """
    logger.info("Calculating cycle time efficiency using APPROVED_CT...")

    df = df.copy()

    # Calculate efficiency: APPROVED_CT / actual_cycle_time
...
def aggregate_per_tool(df: pd.DataFrame) -> pd.DataFrame:
    ...
    tool_metrics = (
        df.groupby(["tool_id", "SUPPLIER_NAME", "TOOLING_TYPE"])
        .agg(
...
def generate_efficiency_summary(tool_metrics: pd.DataFrame) -> Dict:
    ...
    summary = {
        ...
        "top_performers": tool_metrics.nlargest(5, "mean_efficiency")[
            ["tool_id", "SUPPLIER_NAME", "mean_efficiency"]
        ].to_dict("records"),
