...
def extract_daily_instances(
    ...
    Args:
        df: DataFrame with LOCAL_SHOT_TIME, efficiency_pct, CT columns
    ...
    df = df.copy()
    df["shot_hour"] = df["LOCAL_SHOT_TIME"].dt.hour
    ...
    for (date, shift_label), group in df.groupby(["shot_date", "shift_label"]):
        ...
        sorted_group = group.sort_values("LOCAL_SHOT_TIME")
        ...
        instance = DailyShiftInstance(
            ...
            mean_ct=round(float(group["CT"].mean()), 2),
...
def analyze_all_equipment_shifts(
    ...
    Args:
        df: Full DataFrame with efficiency_pct, CT, LOCAL_SHOT_TIME, tool_id
        boundaries: Shift start hours
