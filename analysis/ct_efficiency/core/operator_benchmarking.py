"""
Operator Benchmarking via Cycle Time Behavioral Analysis.
...
def detect_production_sessions(
    ...
    Args:
        equipment_df: DataFrame sorted by LOCAL_SHOT_TIME for one equipment
    ...
    df = equipment_df.sort_values("LOCAL_SHOT_TIME").copy()
    df["time_gap_hours"] = df["LOCAL_SHOT_TIME"].diff().dt.total_seconds() / 3600.0
...
def detect_shift_patterns(
    ...
    Args:
        df: DataFrame filtered to one equipment, with LOCAL_SHOT_TIME and
            efficiency_pct columns
        equipment_code: The equipment identifier

    Returns:
        OperatorShiftPattern describing detected behavioral patterns
    """
    tooling_type = _get_first_value(df, "TOOLING_TYPE", "Unknown")
    ...
    for session in sessions:
        duration = (
            session["LOCAL_SHOT_TIME"].max() - session["LOCAL_SHOT_TIME"].min()
...
def benchmark_operators(
    ...
    Args:
        df: Full DataFrame with efficiency_pct, tool_id, LOCAL_SHOT_TIME,
            TOOLING_TYPE, SUPPLIER_NAME columns
    ...
    for equipment_code, equip_df in equipment_groups:
        equip_df = equip_df.sort_values("LOCAL_SHOT_TIME")
        tooling_type = _get_first_value(equip_df, "TOOLING_TYPE", "Unknown")
        supplier_name = _get_first_value(equip_df, "SUPPLIER_NAME", "Unknown")
