"""
CT Efficiency Analysis API.

This module provides the main API wrapper for CT efficiency and supplier
...
from .core import (
    ...
    benchmark_operators,
    benchmark_suppliers,
    calculate_cycle_time_efficiency,
    compare_tools_by_approved_ct,
...
def run_analysis_api(
    ...
    """Run CT efficiency analysis: supplier benchmarking, operator shift
    analysis (per-supplier boundaries), tool comparison, and staleness detection.

    Args:
        start_date: Start date 'YYYY-MM-DD' (optional)
        end_date: End date 'YYYY-MM-DD' (optional)
        supplier_names: Supplier name filter (optional)
        client: Client/schema name for Snowflake (e.g. 'KESTREL')
        output_dir: Output directory (default: output/ct_efficiency)
    ...
    """
    try:
        logger.info("🚀 Starting CT Efficiency Analysis")
        ...
        if output_dir is None:
            ...
            output_dir = str(get_output_dir("ct_efficiency"))
        else:
        ...
        # Fetch data
        logger.info("📥 Fetching CT efficiency data...")
        ...
        logger.info("🔢 Calculating cycle time efficiency...")
        df = calculate_cycle_time_efficiency(df)
        ...
        all_shift_analyses = []
        for supplier_name, supplier_df in df.groupby("SUPPLIER_NAME"):
            s_name = str(supplier_name)
        ...
        # Tool comparison by approved CT groups
        logger.info("Comparing tools within same approved CT groups...")
        tool_groups = compare_tools_by_approved_ct(df)
        ...
        # Approved CT staleness detection
        ...
        logger.info("✅ CT Efficiency Analysis completed successfully")
        ...
        return {
            ...
            "message": (
                ...
                f"{len(tool_groups)} approved CT groups for tool comparison."
            ),
        }

    except Exception as e:
        logger.error(f"❌ Error in CT efficiency analysis: {e}", exc_info=True)
        return {
            "status": "error",
            "error": str(e),
            "efficiency_summary": {},
...
def _save_html_report(
    ...
    html_content = generate_html_report(
        ...
        efficiency_summary=efficiency_summary,
        supplier_summary=supplier_summary,
        title="CT Efficiency & Supplier Benchmarking Report",
    )
