"""
CT Efficiency Analysis Report Generator.

This module produces a clean, light-themed HTML report covering operator analysis,
tool comparison, and approved CT staleness findings. Designed to look good when
...
<title>CT Efficiency Analysis Report</title>
<style>
...
<body>
<h1>CT Efficiency Analysis Report</h1>
...
def _section_executive_summary(
    ...
    html += "All approved CT baselines are out of date. Machines run 17-43% slower "
...
def _section_operator_analysis(shifts: List) -> str:
    ...
    for a in has_variance:
        v = a.variance
        html += f"""<tr>
        <td><strong>{a.equipment_code}</strong></td>
...
def _section_tool_comparison(tool_groups: List) -> str:
    ...
    html += (
        "<p>Equipment grouped by APPROVED_CT (same target = same or similar parts). "
    )
    html += "Machines compared head-to-head on identical work.</p>"

    for group in tool_groups:
        part_label = ", ".join(group.part_names[:2])
        html += f"<h3>Approved CT: {group.approved_ct}s -- {part_label}</h3>"
        ...
        for t in group.tools:
            ...
            <td><strong>{t.equipment_code}</strong></td>
    ...
    html += "underperform their approved CT target.</div>"
...
def _section_staleness(staleness: List) -> str:
    """Approved CT staleness section."""
    html = '<div class="section-divider"></div>'
    html += "<h2>4. Approved CT Baseline Assessment</h2>"
    html += "<p>Each approved CT is evaluated for staleness based on trend, "
    html += (
        "current performance level, and gap between target and actual cycle time.</p>"
    )

    html += """<table>
    <thead><tr>
      <th>Approved CT</th><th>Parts</th><th>Severity</th>
      <th>Trend/Month</th><th>Current Eff</th><th>Actual CT</th>
      <th>Gap</th><th>Suggested CT</th>
    </tr></thead><tbody>"""
    ...
    for r in staleness:
        ...
        latest_ct = r.monthly_snapshots[-1].mean_ct if r.monthly_snapshots else 0
        gap_pct = round((latest_ct - r.approved_ct) / r.approved_ct * 100, 1)

        html += f"""<tr>
        <td><strong>{r.approved_ct}s</strong></td>
        <td>{', '.join(r.part_names[:2])}</td>
        <td><span class="tag {tag_class}">{r.severity.replace('_', ' ').upper()}</span></td>
        <td>{r.trend_per_month:+.2f}%</td>
        <td>{r.latest_efficiency}%</td>
        <td>{latest_ct}s</td>
        <td>+{gap_pct}%</td>
        <td><strong>{r.suggested_ct}s</strong></td>
        </tr>"""
...
def _section_recommendations(staleness: List) -> str:
    ...
    if stale:
        html += "<li><strong>Recalibrate approved cycle times.</strong> "
        html += 'The following baselines are stale and should be updated:<ul style="margin-top:4px">'
        for r in stale:
            html += (
                f"<li>{r.approved_ct}s &rarr; {r.suggested_ct}s "
                f'({", ".join(r.part_names[:2])})</li>'
            )
        html += "</ul></li>"

    html += "<li><strong>Investigate root causes of CT drift.</strong> "
    html += (
    ...
    html += "Approved CTs should be reviewed quarterly against actual performance "
