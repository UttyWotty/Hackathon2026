"""
Interactive Chart Generator for CT Efficiency Analysis.

This module generates standalone interactive HTML charts using plotly for
operator shift analysis, tool comparison, and approved CT staleness detection.
"""
...
def chart_shift_variance(
    ...
    for a in shift_analyses:
        if a.variance is None:
            continue
        equipment.append(a.equipment_code)
...
def chart_tool_comparison(
    ...
    """Grouped bar chart: equipment efficiency within each approved CT group.
    ...
    fig = make_subplots(
        ...
        cols=1,
        subplot_titles=[
            f"Approved CT: {g.approved_ct}s - {', '.join(g.part_names[:2])}"
    ...
    for i, group in enumerate(display_groups, start=1):
        equip_codes = [t.equipment_code for t in group.tools]
...
def chart_tool_monthly_trend(
    ...
    fig = make_subplots(
        ...
        cols=1,
        subplot_titles=[
            f"Approved CT: {g.approved_ct}s - Monthly Trends" for g in display_groups
        ],
        vertical_spacing=max(0.01, 0.12 / max(1, group_count / 4)),
    )
...
def chart_staleness_overview(
    ...
    """Horizontal bar chart: approved CT vs actual CT with severity coloring.
    ...
    labels = [
        f"{r.approved_ct}s - {', '.join(r.part_names[:2])}" for r in staleness_results
    ]
    ...
    fig.add_trace(
        go.Bar(
            y=labels,
            x=[r.approved_ct for r in staleness_results],
            name="Approved CT",
            orientation="h",
            marker_color=COLORS[0],
            opacity=0.6,
    ...
    fig.add_trace(
        go.Bar(
            y=labels,
            x=[
                r.monthly_snapshots[-1].mean_ct if r.monthly_snapshots else 0
                for r in staleness_results
            ],
            name="Actual Mean CT (Latest Month)",
            orientation="h",
            marker_color=[
    ...
    # Suggested CT markers
    fig.add_trace(
        go.Scatter(
            y=labels,
            x=[r.suggested_ct for r in staleness_results],
            name="Suggested CT",
            mode="markers",
    ...
    fig.update_layout(
        **LAYOUT_DEFAULTS,
        title="Approved CT vs Actual Performance (Staleness Assessment)",
        barmode="group",
        xaxis=dict(title="Cycle Time (seconds)"),
...
def chart_staleness_timeline(
    ...
    """Line chart: group mean efficiency over time for each approved CT group.
    ...
    for i, r in enumerate(staleness_results):
        ...
        label = f"{r.approved_ct}s ({r.severity})"
