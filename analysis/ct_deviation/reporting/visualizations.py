"""
CT Deviation Visualizations.

This module creates charts and visualizations for CT deviation analysis.
...
def create_deviation_distribution_chart(
    ...
    """Create a chart showing CT deviation distribution.
    ...
    try:
        ...
        ax1.set_xlabel("CT Deviation (%)")
        ax1.set_ylabel("Number of Equipment")
        ax1.set_title("CT Deviation Distribution")
        ax1.legend()
...
def create_performance_comparison_chart(
    ...
    try:
        ...
        equipment_codes = [m.equipment_code for m in metrics_list]
...
def create_time_series_chart(
    df: pd.DataFrame, equipment_code: Optional[str] = None
) -> Optional[str]:
    """Create a time series chart showing CT over time for a specific equipment.

    Args:
        df: DataFrame with LOCAL_SHOT_TIME, CT, and APPROVED_CT columns
        equipment_code: Specific equipment to chart (uses most common if None)

    Returns:
        str: Base64-encoded PNG image or None if creation fails
    """
    if df.empty or "LOCAL_SHOT_TIME" not in df.columns:
    ...
    try:
        ...
        if equipment_code is None:
            equipment_code = df["EQUIPMENT_CODE"].value_counts().index[0]

        sample_data = df[df["EQUIPMENT_CODE"] == equipment_code].sort_values(
            "LOCAL_SHOT_TIME"
        )

        if sample_data.empty:
            logger.warning(f"⚠️ No data for equipment {equipment_code}")
            return None
        ...
        # Plot approved CT line
        if "APPROVED_CT" in sample_data.columns:
            approved_ct = sample_data["APPROVED_CT"].iloc[0]
            ax.axhline(
                y=approved_ct,
                color="red",
                linestyle="--",
        ...
        ax.set_title(f"CT Time Series - Equipment {equipment_code}")
        ax.legend()
        ...
        logger.info(f"✅ Created time series chart for {equipment_code}")
        return img_data
...
def create_supplier_comparison_chart(
    ...
    try:
        ...
        for m in metrics_list:
            if m.supplier_name not in supplier_data:
                supplier_data[m.supplier_name] = {
                    "deviations": [],
                    "efficiencies": [],
                    "stabilities": [],
                }
            supplier_data[m.supplier_name]["deviations"].append(
                abs(m.deviation_percentage)
            )
            supplier_data[m.supplier_name]["efficiencies"].append(m.efficiency_score)
            supplier_data[m.supplier_name]["stabilities"].append(m.stability_score)
        ...
        ax1.set_title("Average CT Deviation by Supplier")
        ax1.grid(True, alpha=0.3)
